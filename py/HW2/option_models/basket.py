...
def basket_price_mc(
    ...
    if(bsm):
        ...
        for k in range(n_assets):
            Ft = forward[k] * np.exp(-0.5*texp*vol[k]
                                     ** 2 + np.sqrt(texp)*chol_m[k, :]@znorm_m)
            prices[k, :] = Ft
    else:
...
def basket_price_norm_analytic(
    ...
    cov_m = vol * cor_m * vol[:, None]
    vol_basket = np.sqrt(weights.T @ cov_m @ weights)

    return normal_formula(strike, forward, vol_basket, texp, intr=0.0, divr=0.0, cp=1)
...
def spread_price_kirk(strike, spot, vol, texp, corr, intr=0, divr=0, cp=1):
    ...
    price = disc_fac * \
        bsm_formula(forward[1]+strike, forward[0], vol_r, texp, cp=cp)
